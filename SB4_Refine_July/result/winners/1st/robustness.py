...
def main(args):
    ...
    for i in range(config['num-iterations']):
        ...
        # creathe model and predict
        preds = build_and_predict(train, test, config)
        
        # save the submission in robust.
        preds.to_csv(f"robustness/submission_{i}.csv")
        
        start_date = back_by_1month(start_date)
        end_date = back_by_1month(end_date)
